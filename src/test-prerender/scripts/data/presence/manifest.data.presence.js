/* Manifest Data Sources - Presence & Cursor Tracking */
...
async function subscribeToPresence(channelId, options = {}) {
    ...
    // Current presence state for this user
    let currentCursor = { x: 0, y: 0 };
    let currentFocus = null; // { elementId, elementType, tagName }
    let currentSelection = null; // { start, end, text }
    let currentEditing = null; // { elementId, value, caretPosition }
    let isLocalUserEditing = false; // Track if local user is actively editing
    let lastBroadcastTime = 0;

    // Optimization: Track last sent position and state for change detection
    let lastSentCursor = { x: null, y: null };
    let lastSentFocus = null;
    let lastSentSelection = null;
    let lastSentEditing = null;
    let lastActivityTime = Date.now(); // Track user activity for idle detection
    let lastVelocity = { vx: 0, vy: 0 }; // Velocity for smooth interpolation (future use)
    let lastPosition = { x: 0, y: 0, time: Date.now() }; // For velocity calculation

    // Generate a unique ID for an element if it doesn't have one
    const getElementId = (el) => {
        if (!el) return null;
        if (el.id) return el.id;
        if (el.dataset && el.dataset.presenceId) return el.dataset.presenceId;
    ...
    const handleFocus = (e) => {
        ...
        if (target && element.contains(target)) {
            ...
            lastActivityTime = Date.now(); // Update activity on focus
    ...
    const handleSelectionChange = () => {
        ...
        if (selection && selection.rangeCount > 0 && !selection.isCollapsed) {
            ...
            if (targetElement && element.contains && element.contains(targetElement)) {
                const elementId = getElementId(targetElement);
                if (elementId) {
                    ...
                    if (selectedText && selectedText.trim().length > 0) {
                        ...
                        lastActivityTime = Date.now(); // Update activity on selection
                    } else {
                        currentSelection = null;
                    }
                } else {
    ...
    const handleInput = (e) => {
        ...
        if (target && element.contains(target)) {
            ...
            if (elementId && (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA' || target.isContentEditable)) {
                ...
                isLocalUserEditing = true;
                lastActivityTime = Date.now(); // Update activity on input
            }
        }
    };
    ...
    const findElementById = (elementId) => {
        ...
        if (targetElement && element.contains(targetElement)) return targetElement;
        ...
        if (elementId.includes('>')) {
            ...
            if (current && element.contains(current)) return current;
        }
        return null;
    };

    // Helper function to apply visual indicators for a user's presence state
    const applyVisualIndicators = (userId, focus, selection, editing, userColor) => {
        const color = userColor || getUserColor(userId);

        // Apply focus indicator
        if (focus && focus.elementId) {
            const targetElement = findElementById(focus.elementId);
            if (targetElement) {
        ...
        if (selection && selection.elementId) {
            const targetElement = findElementById(selection.elementId);
            if (targetElement) {
        ...
        if (editing && editing.elementId && editing.caretPosition !== null && editing.caretPosition !== undefined) {
            const targetElement = findElementById(editing.elementId);
            if (targetElement) {
    ...
    const updateElementValue = (targetElement, newValue, caretPosition) => {
    ...
    const updateCursorPosition = async () => {
        ...
        // Throttle broadcasts
        if (now - lastBroadcastTime < throttle) {
            return;
        }
        ...
        if (!hasSignificantChange() && lastSentCursor.x !== null) {
        ...
        try {
            ...
            const presenceData = {
                ...
                lastSeen: now,
                focus: currentFocus ? JSON.stringify(currentFocus) : null,
                selection: currentSelection ? JSON.stringify(currentSelection) : null,
                editing: currentEditing ? JSON.stringify(currentEditing) : null
            };
            ...
            lastSentCursor = { x: currentCursor.x, y: currentCursor.y };
            lastSentFocus = currentFocus ? JSON.parse(JSON.stringify(currentFocus)) : null;
            lastSentSelection = currentSelection ? JSON.parse(JSON.stringify(currentSelection)) : null;
            lastSentEditing = currentEditing ? JSON.parse(JSON.stringify(currentEditing)) : null;

            // Debug: Log when focus/editing data is being sent
    ...
    const updateInterval = setInterval(updateCursorPosition, throttle);
    ...
    // Declare subscriptionResult outside try block so it's accessible after
    let subscriptionResult = null;

    try {
        // Create callback function
        const presenceCallback = (response) => {
            ...
            events.forEach(event => {
                ...
                if (event.includes('create') || event.includes('rows.create')) {
                    ...
                    if (payload.channelId === channelId) {
                        ...
                        applyVisualIndicators(userId, focus, selection, editing, userColor);
                    ...
                    if (payload.channelId === channelId) {
                        ...
                        if (payload.editing !== undefined) {
                            try {
                                ...
                                if (editing && editing.elementId && editing.value !== undefined) {
                                    ...
                                    const isLocalEditingThis = isLocalUserEditing &&
                                        currentEditing &&
                                        currentEditing.elementId === editing.elementId;
                        ...
                        applyVisualIndicators(userId, focus, selection, editing, existing.color || getUserColor(userId));
        ...
        // Appwrite SDK subscribe - match EXACT pattern from working subscribeToTable
        // The callback is registered immediately when subscribe() is called, not when Promise resolves

        // Call subscribe exactly like the working code does - callback registered immediately
        const unsubscribeResult = services.realtime.subscribe(presenceChannel, presenceCallback);

        // Handle Promise if it's returned (for unsubscribe function), but callback is already registered
        if (unsubscribeResult && typeof unsubscribeResult.then === 'function') {
            // Callback is already registered, Promise is just for unsubscribe function
            unsubscribeResult.then((resolved) => {
                if (typeof resolved === 'function') {
                    unsubscribe = resolved;
                } else if (resolved && typeof resolved.close === 'function') {
                    unsubscribe = () => resolved.close();
                }
            }).catch((error) => {
            });
            // Set temporary unsubscribe
            unsubscribe = () => {
                if (unsubscribeResult && typeof unsubscribeResult.then === 'function') {
                    unsubscribeResult.then((resolved) => {
        ...
        } else if (typeof unsubscribeResult === 'function') {
            unsubscribe = unsubscribeResult;
        } else if (unsubscribeResult && typeof unsubscribeResult.close === 'function') {
            unsubscribe = () => unsubscribeResult.close();
        } else {
        }

        subscriptionResult = unsubscribeResult; // Store for logging
            ...
            if (allCursors && allCursors.rows) {
                allCursors.rows.forEach(row => {
                    if (row.userId && row.userId !== userInfo.id) {
                        ...
                        applyVisualIndicators(row.userId, focus, selection, editing, userColor);
                    }
                });
